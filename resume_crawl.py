...
async def main():
    ...
    try:
        ...
        for plat, cfg in kws.items():
            ...
            for kw, cat, mp in cfg["keywords"]:
                cur += 1
                actual_mp = min(mp, 2)  # 每个关键词最多2页
                print(f"\n  [{cur}/{total}] 🔍 {kw} (最多{actual_mp}页)")

                # 检查浏览器是否存活，但不重建（避免再次扫码）
    ...
    print(f"\n{'='*55}")
    print(f"🎉 爬取完成！新增 {len(notes)} 条")
    print(f"   保存到: {OUT}")
    print(f"   接下来运行: python3 merge_and_build.py")
    print(f"{'='*55}")
